...
% Propagation vers le plan focal (champ complet)
F_full = fftshift(fft2(ComplexDevice));
Ffilt   = F_full .* Hfull;          % <-- applique le pinhole

% Intensité PSF
PSF = abs(Ffilt).^2;
PSF = PSF / max(PSF(:));   % normalisation

% Coordonnées fréquentielles
fx_psf = (-Ndmd/2:Ndmd/2-1) / Ndmd;

figure('Position',[100 100 1200 500])

subplot(1,2,1)
imagesc(fx_psf, fx_psf, PSF)
axis image; colorbar; colormap(hot)
title('PSF (échelle linéaire)')
xlabel('f_x [cycles/pixel]')
ylabel('f_y [cycles/pixel]')
% Zoom sur l'ouverture
xlim([-3*r 3*r]); ylim([-3*r 3*r])
